...
[![الإصدار](https://img.shields.io/badge/version-0.24.0-2563eb?style=flat-square&logo=semver)](../CHANGELOG.md)
[![الإصدار](https://img.shields.io/badge/version-0.24.1-2563eb?style=flat-square&logo=semver)](../CHANGELOG.md)
...
مصفوفة الإصدار: `0.24.0` (مساحة العمل)، `versionCode = 70` (Android). ملاحظة: keystore إصدار Android لم تعد مضمنة في المستودع - راجع SECURITY.md؛ وفّر `ORBISCREEN_KEYSTORE_PATH`/`ORBISCREEN_STORE_PASSWORD`/`ORBISCREEN_KEY_ALIAS`/`ORBISCREEN_KEY_PASSWORD` عند بناء APK الإصدار.
مصفوفة الإصدار: `0.24.1` (مساحة العمل)، `versionCode = 71` (Android). ملاحظة: keystore إصدار Android لم تعد مضمنة في المستودع - راجع SECURITY.md؛ وفّر `ORBISCREEN_KEYSTORE_PATH`/`ORBISCREEN_STORE_PASSWORD`/`ORBISCREEN_KEY_ALIAS`/`ORBISCREEN_KEY_PASSWORD` عند بناء APK الإصدار.
...
عند دفع وسم إصدار (مثلاً `git tag v0.24.0 && git push origin v0.24.0`)، يبني سير العمل `.github/workflows/release.yml` تلقائياً جميع حزم الإصدار ويرفقها بصفحة GitHub Releases.
عند دفع وسم إصدار (مثلاً `git tag v0.24.1 && git push origin v0.24.1`)، يبني سير العمل `.github/workflows/release.yml` تلقائياً جميع حزم الإصدار ويرفقها بصفحة GitHub Releases.
 
يُولَّد محتوى `body` للإصدار من كتلة `## [v0.24.0]` في `CHANGELOG.md`.
يُولَّد محتوى `body` للإصدار من كتلة `## [v0.24.1]` في `CHANGELOG.md`.
